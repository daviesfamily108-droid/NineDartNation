...
describe('applyVisitCommit', () => {
  beforeEach(() => {
    ...
    useMatch.setState({ players: [], currentPlayerIdx: 0, startingScore: 501, inProgress: false })
  })
  ...
  it('does not double-apply duplicate visits', () => {
    useMatch.getState().newMatch(['A', 'B'], 501)
  const first = applyVisitCommit(useMatch.getState(), { value: 60, darts: 3 })
  expect(first.applied).toBeTruthy()
  const second = applyVisitCommit(useMatch.getState(), { value: 60, darts: 3 })
