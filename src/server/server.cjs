...
// Avatars are stored as base64 data URIs keyed by username.
...
function loadAvatarsFromDisk() {
  ...
  try {
    ...
    startLogger.warn('[Avatars] Failed to load from disk:', err && err.message);
    avatarsCache = {};
  }
  return avatarsCache;
}
...
function persistAvatarsToDisk() {
  try {
    ...
    startLogger.warn('[Avatars] Failed to persist to disk:', err && err.message);
  }
}
...
async function ensureFriendshipsTable() {
  if (!supabase) return
  try {
    // Try a test upsert with a dummy row to see if the constraint exists.
    // If the table doesn't exist at all, Supabase will return a specific error.
    ...
    if (error) {
      ...
      // If the error mentions the table doesn't exist or constraint is missing,
      // try creating the table via RPC (requires Supabase SQL access)
