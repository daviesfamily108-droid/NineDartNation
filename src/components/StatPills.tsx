...
export default function StatPills({ items, onUnlock }: { items: StatItem[]; onUnlock?: () => void }) {
  ...
  const getTextSizeClasses = (size: string) => {
    switch (size) {
      case 'small': return { label: 'text-[10px]', value: 'text-sm' }
      case 'large': return { label: 'text-sm', value: 'text-xl' }
      default: return { label: 'text-[12px]', value: 'text-lg' }
    }
  }
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {items.map((it) => (
        <div key={it.key} className={`relative ${boxSizeClass} rounded-xl border ${it.locked ? 'border-white/10 bg-white/5' : 'border-indigo-500/40 bg-indigo-500/10'}`}>
          <div className={`${textClasses.label} mb-1 ${it.locked ? 'opacity-50' : 'opacity-80'}`}>{it.label}</div>
          <div className={`${textClasses.value} font-semibold ${it.locked ? 'opacity-50 blur-[1px]' : ''}`}>{it.value}</div>
          {it.locked && (
            <button
