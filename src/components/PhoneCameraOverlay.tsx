...
export default function PhoneCameraOverlay() {
  ...
  const hasLiveVideo = !!(
    videoElement &&
    ((videoElement as any).srcObject || (videoElement as any).readyState >= 2)
  );
  ...
  const shouldShow =
    ...
    (cameraSession.isStreaming || hasLiveVideo) &&
    !!videoElement;
  ...
  useEffect(() => {
    ...
    if (!shouldShow) {
      console.warn(
        "[PhoneCameraOverlay] NOT SHOWING - At least one condition failed",
      );
    } else {
      console.log(
        "[PhoneCameraOverlay] ✓ ALL CONDITIONS MET - Should display camera overlay",
      );
    }
  }, [
    shouldShow,
    ...
    const renderFrame = () => {
      if (sourceVideo && ctx) {
        ...
        try {
          ...
          console.debug(
            "[PhoneCameraOverlay] Canvas render frame error (expected during stream setup):",
            e,
          );
        }
      }
  ...
  useEffect(() => {
    const handler = () => {
      try {
        cameraSession.setShowOverlay(!cameraSession.showOverlay);
      } catch {}
    };
    window.addEventListener("ndn:toggle-phone-overlay", handler);
    return () => {
      window.removeEventListener("ndn:toggle-phone-overlay", handler);
    };
  }, [cameraSession.showOverlay]);
  ...
  return (
    <div
      ...
      className="fixed z-40 bg-black rounded-lg shadow-xl overflow-hidden border border-blue-500 ndn-overlay"
      style={{
        left: `${position.x}px`,
        top: `${position.y}px`,
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="text-white hover:bg-blue-800 px-2 py-0.5 rounded text-xs font-bold transition-colors ndn-overlay-interactive"
      ...
      {!minimized && (
        <div
          ...
          className="bg-black w-full relative overflow-hidden ndn-overlay-interactive"
