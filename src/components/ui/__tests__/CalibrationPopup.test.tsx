...
describe('CalibrationPopup', () => {
  ...
  test('renders players and provides Skip and Bull Up actions', () => {
    const players = [
    ...
    const onClose = vi.fn()
    render(<CalibrationPopup players={players} playerCalibrations={{}} calibrationSkipped={{}} onSkip={onSkip} onOpenCalibrator={onOpenCalibrator} onClose={onClose} />)
  ...
  test('Start Match button is disabled until all players skipped', () => {
    const players = [
      { id: '1', name: 'Alice', legsWon: 0, legs: [] },
      { id: '2', name: 'Bob', legsWon: 0, legs: [] },
    ] as any
    ...
    const onOpenCalibrator = vi.fn()
    const onClose = vi.fn()
    const { rerender } = render(<CalibrationPopup players={players} playerCalibrations={{}} calibrationSkipped={{}} onSkip={onSkip} onOpenCalibrator={onOpenCalibrator} onClose={onClose} />)
    // Start button disabled
    const startBtn = screen.getByRole('button', { name: /Start Match/i })
    expect(startBtn).toBeDisabled()

    // Mark both players as skipped: simulate props change
    rerender(<CalibrationPopup players={players} playerCalibrations={{}} calibrationSkipped={{ '1': true, '2': true }} onSkip={onSkip} onOpenCalibrator={onOpenCalibrator} onClose={onClose} />)
