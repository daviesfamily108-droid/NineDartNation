          ...
          {players.map(player => (
            <div key={player.id} className="flex items-center justify-between p-4 bg-slate-700/50 rounded-lg">
              ...
              <div className="flex gap-2">
                ...
                <button className="btn btn-sm btn-primary" onClick={() => onSkip(player.id)} disabled={!!calibrationSkipped[player.id]}>{calibrationSkipped[player.id] ? 'Skipped ✓' : 'Skip'}</button>
              </div>
            </div>
          ))}
        </div>
