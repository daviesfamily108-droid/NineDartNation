...
export default function MatchControls({ inProgress = true, startingScore = 501, pendingEntries = [], onAddVisit, onUndo, onEndLeg, onNextPlayer, onEndGame, showDartsSelect = true, quickButtons = [180,140,100,60] }: MatchControlsProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          className="btn"
          onClick={() => { onAddVisit(Math.max(0, Number.isFinite(score) ? score : 0), darts); setScore(0) }}
        >Add Visit</button>
        <button className="px-3 py-2 rounded-xl border border-slate-200" onClick={() => onUndo && onUndo()} title="Undo">
      ...
      {quickButtons && quickButtons.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mt-2 text-xs">
          ...
          {quickButtons.map(v => (
            <button key={String(v)} className="btn px-2 py-0.5 text-xs" onClick={() => onAddVisit(v, 3)}>{v}</button>
          ))}
        </div>
      )}
      <div className="flex flex-wrap items-center gap-2">
