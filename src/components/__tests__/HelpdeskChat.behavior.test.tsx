...
vi.mock('../WSProvider', () => ({
  useWS: () => ({
    ...
    addListener: (fn: any) => { listener = fn; return () => { listener = null } }
  })
}))
...
describe('HelpdeskChat behavior', () => {
  ...
  it('shows timestamps and reacts to typing events', async () => {
    const now = Date.now()
    const req = { id: 'r-123', username: 'tester', messages: [{ fromName: 'User', message: 'Hi', ts: now - 60000, admin: false }] }
    const user = { email: 'admin@example.com', username: 'admin', isAdmin: true }

    render(<HelpdeskChat request={req} user={user} onClose={() => {}} />)

    // timestamp should be visible in HH:MM format
    const timeString = new Date(now - 60000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
