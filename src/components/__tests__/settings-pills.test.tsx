...
describe("Settings Panel pill open/close", () => {
  it("User Info pill opens and closes immediately", async () => {
    ...
    const r = render(
      <ThemeProvider>
        <SettingsPanel
          user={{ username: "test", email: "a@b.c", usernameChangeCount: 0 }}
        />
      </ThemeProvider>,
    );
    ...
    await waitFor(() =>
      expect(r.queryByTestId("pill-user-content")).toBeNull(),
    );
  });
  ...
  it("Settings pill opens and closes immediately", async () => {
    ...
    await waitFor(() =>
      expect(r.queryByTestId("pill-settings-content")).toBeNull(),
    );
  });
});
