...
describe("Sidebar cached subscription fallback", () => {
  ...
  it("hides PREMIUM tab when localStorage cached subscription is active", () => {
    ...
    try {
      localStorage.setItem(`ndn:subscription:${email}`, JSON.stringify(cached));
    } catch {}
    ...
    if (typeof rawGet === "function") {
      expect(
        rawGet.call(localStorage, `ndn:subscription:${email}`),
      ).not.toBeNull();
    } else {
      ...
      const maybeRaw =
        (localStorage as any)._store || (localStorage as any).store || null;
      // If present, check our key; otherwise we can't rely on the stub.
      if (maybeRaw)
        expect(
          Object.prototype.hasOwnProperty.call(
            maybeRaw,
            `ndn:subscription:${email}`,
          ),
        ).toBe(true);
    }
