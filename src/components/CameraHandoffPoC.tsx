    ...
    try {
      ...
      const tick = async () => {
        ...
        try {
          ...
          try {
            channelRef.current?.postMessage({ type: "frame", bitmap });
          } catch {}
        } catch {}
    ...
    <div className="card p-3">
      ...
      <div className="flex gap-2 mt-2">
        {!running ? (
          ...
          <button className="btn bg-rose-600" onClick={stop}>
            Stop
          </button>
        )}
        <div className="text-sm opacity-70">
          Note: frame transfer across tabs is experimental and may be blocked by
          browser security.
        </div>
      </div>
    </div>
