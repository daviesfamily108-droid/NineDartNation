...
describe('broadcast fallback helpers', () => {
  it('subscribe receives messages from broadcastMessage via localStorage fallback', async () => {
    ...
    try {
      ...
      try { (global as any).BroadcastChannel = class { constructor(){ throw new Error('force-fallback'); } }; } catch {}
      try { (window as any).BroadcastChannel = (global as any).BroadcastChannel; } catch {}
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        try {
          window.dispatchEvent(new CustomEvent('ndn:match-sync', { detail: { msg: payload } }));
        } catch (e) {
          try {
      ...
      try { (global as any).BroadcastChannel = OriginalGlobalBC; } catch {}
      try { (window as any).BroadcastChannel = OriginalWindowBC; } catch {}
    }
  });
});
