...
export function parseVoiceInput(transcript: string): VoiceResult {
  ...
  // Check commands
  if (VOICE_COMMANDS.UNDO.some((cmd) => clean.includes(cmd)))
    return { type: "command", value: "undo" };
  if (VOICE_COMMANDS.BUST.some((cmd) => clean.includes(cmd)))
    return { type: "command", value: "bust" };
  if (VOICE_COMMANDS.NEXT.some((cmd) => clean.includes(cmd)))
    return { type: "command", value: "next" };
