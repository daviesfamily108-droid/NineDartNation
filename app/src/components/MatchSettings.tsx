  ...
  return (
    <div className="card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className="block text-sm text-slate-600 mb-1">Players (comma separated)</label>
          <input className="input w-full" value={playersText} onChange={e => setPlayersText(e.target.value)} />
        </div>
        <div>
          <label className="block text-sm text-slate-600 mb-1">Starting Score</label>
          <input className="input w-full" type="number" value={start} onChange={e => setStart(parseInt(e.target.value || '501'))} />
        </div>
        <div>
          <label className="block text-sm text-slate-600 mb-1">Room ID (optional)</label>
          <input className="input w-full" value={room} onChange={e => setRoom(e.target.value)} placeholder="room-1" />
        </div>
      </div>
      <div className="mt-3">
